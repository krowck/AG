  ...
  { int loop;
    ...
    { if (j==nl && i==nr)
        ...
        { int k;
          double xmax = x[0];
          double xmin = x[n-1];
          for (k = lo; k <= j; k++) xmax = _max(xmax,x[k]);
          for (k = i; k <= hi; k++) xmin = _min(xmin,x[k]);
...
{ int j;
  ...
  { int is = 1;
    ...
    /* Fix the distances */
    for (j = 0; j < js; j++)
      distmatrix[js][j] = _max(distmatrix[is][j],distmatrix[js][j]);
    for (j = js+1; j < is; j++)
      distmatrix[j][js] = _max(distmatrix[is][j],distmatrix[j][js]);
    for (j = is+1; j < n; j++)
      distmatrix[j][js] = _max(distmatrix[j][is],distmatrix[j][js]);
